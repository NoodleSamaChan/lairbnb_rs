...
use crate::{authentication::Credentials, domain::{SubscriberName, SubscriberPassword}};
...
pub fn create_cookie(username: &SubscriberName, password: &SubscriberPassword,) -> String {
    ...
    let final_cookie: Vec<u8> = cookie.bytes().zip(secret.bytes().cycle()).map(|(cookie, secret)| cookie ^ secret).collect();
...
pub fn extract_cookie(request: HttpRequest,) -> Result<UserInfo, anyhow::Error> {
    ...
    let final_cookie: Vec<u8> = decoded_image_data.iter().zip(secret.bytes().cycle()).map(|(decoded_image_data, secret)| decoded_image_data ^ secret).collect();
